...
对于多进程的设计我们可以先推测一下：
- 进程回收：通过oom_adj值和进程占用的内存大小两个值共同决定回收哪些进程
- 进程并发调度：通过nice值标记进程调度优先级

所以对于像音乐播放器这样的后台进程存活情况如何，怎么提高其存活率是我们需要关注的点。这里可以直接说一下，`使用LRU算法+组件特点排序进程;使用oom_adj值和占用内存大小回收进程`
